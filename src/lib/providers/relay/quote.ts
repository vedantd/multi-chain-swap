...
export async function calculateWorstCaseCosts(
  ...
  if (connection && params.originChainId === CHAIN_ID_SOLANA) {
    try {
      ...
      if (tokenInfo.hasTransferFees && tokenInfo.transferFeeBps != null && tokenInfo.transferFeeBps > 0) {
        const feeBps = tokenInfo.transferFeeBps;
        tokenLossUsd = expectedOutUsd * (feeBps / 10_000);
      }
    } catch {
      tokenLossUsd = 0;
    }
  ...
  // Failure buffer: assume up to 2 failed transactions (3x gas cost)
